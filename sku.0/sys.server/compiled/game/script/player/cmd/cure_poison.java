...
/**
 * Publish 14.1 Cure Poison compatibility adapter.
 *
 * The retained healing library owns poison-strength reduction, medicine
 ...
 */
public class cure_poison extends script.base_script
{
    ...
    private static final string_id SID_SELF_NOT_POISONED =
        new string_id(
            "healing_response",
            "healing_response_82");
    private static final string_id SID_TARGET_NOT_POISONED =
        new string_id(
            "healing_response",
            "healing_response_84");
        ...
        throws InterruptedException
    {
        ...
        if (!dot.isPoisoned(target))
        {
            if (self == target)
            {
                sendSystemMessage(self, SID_SELF_NOT_POISONED);
            }
            else if (isPlayer(target))
            {
                prose_package message =
                    prose.getPackage(
                        SID_TARGET_NOT_POISONED,
                        target);
                sendSystemMessageProse(self, message);
            }
            recordOutcome(self, fixture, "notPoisoned");
            return SCRIPT_CONTINUE;
        }
        ...
        int poisonBefore =
            getPoisonStrength(target);
        int power =
            healing.getDotPower(medicine);
        boolean performed =
            healing.performCurePoison(
                self,
                target,
                medicine);
        if (!performed)
        ...
        if (fixture)
        {
            ...
            setObjVar(
                self,
                FIXTURE_ROOT + ".poisonBefore",
                poisonBefore);
            setObjVar(
                self,
                FIXTURE_ROOT + ".poisonAfter",
                poisonAfter);
            setObjVar(
                self,
                FIXTURE_ROOT + ".poisonReduction",
                poisonBefore -
                    Math.max(0, poisonAfter));
            setObjVar(
                self,
        ...
        throws InterruptedException
    {
        return
            isIdValid(medicine) &&
            exists(medicine) &&
            healing.isCurePoisonMedicine(medicine) &&
    ...
    private int getPoisonStrength(obj_id target)
        throws InterruptedException
    {
        String[] poison =
            dot.getAllDotsType(
