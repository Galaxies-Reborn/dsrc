...
/**
 * Publish 14.1 organic tend-damage command.
 *
 * Tending intentionally uses no medicine object. It heals Health and Action
 * from skill and patient battle fatigue, then charges Mind and adds Focus and
 * Willpower wounds.
 */
public class tend_damage extends script.base_script
{
        ...
        throws InterruptedException
    {
        ...
        if (getAttrib(target, HEALTH) >=
                getWoundedMaxAttrib(target, HEALTH) &&
            getAttrib(target, ACTION) >=
                getWoundedMaxAttrib(target, ACTION))
        {
        ...
        if (fixture)
        {
            setObjVar(
                self,
                FIXTURE_ROOT + ".tendDamageExpectedMindCost",
                healing.getTendMindCost(self, false));
        }
        boolean performed =
            healing.performTendDamage(self, target, true);
        if (performed)
        {
            doAnimationAction(
                self,
        ...
        if (fixture)
        {
            setObjVar(
                self,
                FIXTURE_ROOT + ".tendDamageHealthHeal",
                getAttrib(target, HEALTH) - healthBefore);
            setObjVar(
                self,
                FIXTURE_ROOT + ".tendDamageActionHeal",
                getAttrib(target, ACTION) - actionBefore);
            setObjVar(
                self,
                FIXTURE_ROOT + ".tendDamageMindCost",
                mindBefore - getAttrib(self, MIND));
            setObjVar(
                self,
                FIXTURE_ROOT + ".tendDamageFocusWoundCost",
                getAttribWound(self, FOCUS) - focusWoundBefore);
            setObjVar(
                self,
                FIXTURE_ROOT + ".tendDamageWillpowerWoundCost",
                getAttribWound(self, WILLPOWER) -
                    willpowerWoundBefore);
            setObjVar(
                self,
                FIXTURE_ROOT + ".tendDamageMedicalXp",
        ...
        throws InterruptedException
    {
        ...
        setObjVar(
            healer,
            FIXTURE_ROOT + ".tendDamageEntered",
            getGameTime());
        ...
        throws InterruptedException
    {
        if (fixture)
        {
            setObjVar(
                healer,
                FIXTURE_ROOT + ".tendDamageOutcome",
                outcome);
        }
    }
