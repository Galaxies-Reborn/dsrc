...
/**
 * Identity-bound and reversible fixture for authentic novice Doctor.
 */
public class precu_doctor_novice_fixture extends script.base_script
{
    private static final long PLAYER_OID = 39008597L;
    private static final int PLAYER_STATION_ID = 1001;
    private static final String ROOT = "precu.doctorNoviceFixture";
    ...
    private static final String NOVICE = "science_doctor_novice";
        ...
        throws InterruptedException
    {
        ...
        if (!setXpExact(player, MEDICAL_XP, 11250))
        {
            restore(player);
            removeObjVar(player, ROOT);
            return "error=medicalXpPrepareFailed";
        }
        ...
        throws InterruptedException
    {
        ...
        boolean purchased = purchaseWithoutHolocron(player, NOVICE);
        return "action=purchase novice=" + purchased +
            " " + buildStatus(player);
    }
        ...
        throws InterruptedException
    {
        ...
        return "player=" + player +
            " prerequisites=" + buildBits(player, PREREQUISITES) +
            " novice=" + (hasSkill(player, NOVICE) ? "1" : "0") +
            " commands=" + buildBits(player, COMMANDS) +
            " schematics=" + buildBits(player, SCHEMATICS) +
            " modDeltas=" + deltas +
    ...
    private String buildBits(obj_id player, String[] values)
        throws InterruptedException
    {
        String bits = "";
        for (int index = 0; index < values.length; ++index)
        {
            boolean owned = values == PREREQUISITES
                ? hasSkill(player, values[index])
                : values == COMMANDS
                    ? hasCommand(player, values[index])
                    : hasSchematic(player, values[index]);
            bits += owned ? "1" : "0";
        }
        return bits;
    }
