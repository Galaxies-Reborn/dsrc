...
/**
 * Identity-bound and reversible fixture for authentic Doctor tier III.
 *
 * Preparation owns the complete Medic through Doctor tier-II prerequisite
 * vector. Purchase invokes the production validation, grant, and XP-deduction
 * operations for all four tier-III boxes. ServerConsole has no owner context,
 * so only the post-purchase holocron notification is intentionally omitted.
 */
public class precu_doctor_tier3_fixture extends script.base_script
{
    private static final long PLAYER_OID = 39008597L;
    private static final int PLAYER_STATION_ID = 1001;
    private static final String ROOT = "precu.doctorTier3Fixture";
    ...
    private static final int PREPARED_MEDICAL_XP = 45000;
    private static final int PREPARED_CRAFTING_XP = 27000;
        ...
        throws InterruptedException
    {
        ...
        boolean wound = purchaseWithoutHolocron(player, TIER_THREE[0]);
        boolean speedXp =
        ...
        throws InterruptedException
    {
        ...
        for (int index = TIER_THREE.length - 1; index >= 0; --index)
        {
            if (hasSkill(player, TIER_THREE[index]))
            {
                revokeSkill(player, TIER_THREE[index]);
            }
        }
        revokePrerequisites(player);
        boolean xpRestored =
        ...
        throws InterruptedException
    {
        ...
        return "player=" + player +
            ...
            " tierSkills=" + buildSkillBits(player, TIER_THREE) +
        ...
        throws InterruptedException
    {
        return containsSkill(player, PREREQUISITES) ||
            containsSkill(player, TIER_THREE);
    }
