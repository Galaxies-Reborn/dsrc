...
/**
 * Identity-bound and reversible fixture for authentic Doctor tier II.
 *
 * Preparation owns the complete Medic through Doctor tier-I prerequisite
 * vector.
 * Purchase invokes the production validation, grant, and XP-deduction
 * operations for all four tier-II boxes. ServerConsole has no owner context,
 * so only the post-purchase holocron notification is intentionally omitted.
 */
public class precu_doctor_tier2_fixture extends script.base_script
{
    private static final long PLAYER_OID = 39008597L;
    private static final int PLAYER_STATION_ID = 1001;
    private static final String ROOT = "precu.doctorTier2Fixture";
    ...
    private static final int PREPARED_MEDICAL_XP = 30000;
    private static final int PREPARED_CRAFTING_XP = 21000;
        ...
        throws InterruptedException
    {
        ...
        return "player=" + player +
            ...
            " tierSkills=" +
                buildSkillBits(player, TIER_TWO) +
            " commands=" +
                buildCommandBits(player, EXPECTED_COMMANDS) +
            " ngeCommands=" +
    ...
    private boolean hasAnyCommand(obj_id player)
        throws InterruptedException
    {
        for (int index = 0; index < EXPECTED_COMMANDS.length; ++index)
        {
            if (hasCommand(player, EXPECTED_COMMANDS[index]))
            {
                return true;
            }
        }
    ...
    private boolean hasAnySchematic(obj_id player)
        throws InterruptedException
    {
        for (int index = 0; index < PREREQUISITE_SCHEMATICS.length; ++index)
        {
            if (hasSchematic(player, PREREQUISITE_SCHEMATICS[index]))
            {
                return true;
            }
        }
